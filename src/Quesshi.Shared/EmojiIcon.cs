...
public static class EmojiIcon
{
    public static bool TryNormalize(string? value, out string normalized)
    {
        ...
        if (IsKeycap(runes)) return true;

        for (var i = 0; i < runes.Length;)
        {
            var emoji = runes[i++];
            if (!IsEmoji(emoji) || IsRegionalIndicator(emoji) || IsModifier(emoji)) return false;

            var hasEmojiSelector = i < runes.Length && runes[i].Value == 0xFE0F;
            if (hasEmojiSelector) i++;
            if (!hasEmojiSelector && !IsEmojiPresentation(emoji)) return false;

            if (i < runes.Length && IsModifier(runes[i])) i++;
            if (i == runes.Length) return true;
            if (runes[i++].Value != 0x200D || i == runes.Length) return false;
        }

        return true;
    }

    private static bool IsKeycap(Rune[] runes)
    {
        if (runes.Length is not (2 or 3) || runes[^1].Value != 0x20E3) return false;
        if (runes.Length == 3 && runes[1].Value != 0xFE0F) return false;
        return runes[0].Value is 0x23 or 0x2A or >= 0x30 and <= 0x39;
    }
    ...
    private static bool IsEmoji(Rune rune) => InRanges(rune.Value, EmojiRanges);
    private static bool IsEmojiPresentation(Rune rune) => InRanges(rune.Value, EmojiPresentationRanges);
    ...
    // Unicode 16 emoji-data.txt. Keeping the property ranges here gives the WebAssembly client and
    // server the same validation without relying on platform-specific ICU APIs.
