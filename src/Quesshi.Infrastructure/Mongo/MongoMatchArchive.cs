...
public sealed class MongoMatchArchive(MongoContext db) : IMatchArchive
{
    ...
    public async Task<IReadOnlyList<ArchivedMatch>> ForPlayerAsync(string playerId, int take, CancellationToken ct = default)
        => [.. (await db.Matches
            // One AnyEq over the multikey Participants index replaces the old Or(Eq(ChallengerId),
            // Eq(OpponentId)) — the same rows, however many seats a player has ever held in a match,
            // served by the single index MongoContext.EnsureIndexesAsync now builds.
            .Find(Builders<MatchDoc>.Filter.AnyEq(m => m.Participants, playerId))
